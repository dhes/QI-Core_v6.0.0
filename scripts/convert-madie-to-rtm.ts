...
class MADiEToRTMConverter {
  ...
  async convertTestSuite(config: TransformConfig): Promise<void> {
    ...
    console.log(`\n✅ Conversion complete!`);
    console.log(`Converted ${testCases.length} test cases to ${config.targetDir}`);
  }
  ...
  private async discoverResources(testCasePath: string): Promise<Map<string, FHIRResource[]>> {
  ...
  private generateUUIDs(testCases: TestCase[]): void {
    for (const testCase of testCases) {
      ...
      this.referenceMap.set(`Patient/${testCase.patientId}`, `Patient/${newPatientId}`);
      ...
      for (const [resourceType, resources] of testCase.resources) {
        for (const resource of resources) {
          ...
          this.referenceMap.set(`${resourceType}/${resource.id}`, `${resourceType}/${newId}`);
        }
      }
    }
  }

  private async transformTestCase(testCase: TestCase, config: TransformConfig): Promise<void> {
    ...
    for (const [resourceType, resources] of testCase.resources) {
      ...
      for (const resource of resources) {
        const transformedResource = this.transformResource(resource, config.yearOffset);
        const newId = this.uuidMap.get(resource.id)!;
        const newFilePath = path.join(newResourceDir, `${newId}.json`);
        
        await fs.writeFile(newFilePath, JSON.stringify(transformedResource, null, 2));
      }
    }
  }
  ...
  private rollBackDateString(dateStr: string, yearOffset: number): string {
    try {
      ...
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
        ...
        const originalFormat = dateStr.includes('Z') ? dateStr : dateStr.replace('Z', '');

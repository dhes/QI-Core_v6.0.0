...
async function runAllTestCases() {
  ...
  for (const dir of testCaseDirs) {
    ...
    try {
      ...
      let cqlResults = raw.patientResults?.[patientId];

      if (!cqlResults || Object.keys(cqlResults).length === 0) {
        patientId = Object.keys(raw.unfilteredResults || {})[0];
        cqlResults = raw.unfilteredResults?.[patientId];
        if (!cqlResults) {
          console.warn(`No results found for: ${dir}`);
          continue;
        }
      }
      ...
      for (const flag of interestingFlags) {
        const key = flag.replace(/\s+/g, "");
        row[key] = !!cqlResults[flag];
      }
      results.push(row);
